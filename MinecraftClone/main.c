...
int main() {
    ...
    LoadWorld();
    ReloadMesh();
    Shader shader = LoadShader("assets/shaders/vertex.glsl","assets/shaders/fragment.glsl");
    shader.locs[SHADER_LOC_VECTOR_VIEW] = GetShaderLocation(shader, "viewPos");
    Texture2D texture = LoadTexture("assets/atlas.png");
    ...
    Image img = LoadImage("assets/breaking.png");
    Texture2D otherTexture = LoadTexture("assets/other.png");
...
void SaveWorld() {
    FILE *file = fopen("assets/world.bin", "wb");
    if (file == NULL) {
...
void LoadWorld() {
    FILE *file = fopen("assets/world.bin", "rb");
    if (file == NULL) {
        for (int x = 0; x < CHUNK_SIZE; x++) {
            for (int z = 0; z < CHUNK_SIZE; z++) {
                for (int y = 0; y < 4; y++) {
